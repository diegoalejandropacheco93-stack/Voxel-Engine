...
class PlayState extends MusicBeatState
{
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				var introAlts:Array<String> = introAssets.get(stageUI);
				// --- VOXEL ENGINE OPTIMIZATION: lowQualityStage ---
				var antialias:Bool = (ClientPrefs.data.antialiasing && !isPixelStage && !ClientPrefs.data.lowQualityStage);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		// --- VOXEL ENGINE OPTIMIZATION: disableSparks ---
		if(daRating.noteSplash && !note.noteSplashData.disabled && !ClientPrefs.data.disableSparks)
		...
		{
			uiFolder = uiPrefix + "UI/";
			antialias = !isPixelStage && !ClientPrefs.data.lowQualityStage;
		}
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		else //Notes that count as a miss if you hit them (Hurt notes for example)
		{
			...
			// --- VOXEL ENGINE OPTIMIZATION: disableSparks ---
			if(!note.noteSplashData.disabled && !note.isSustainNote && !ClientPrefs.data.disableSparks) spawnNoteSplashOnNote(note);
		}
		...
		for (name in achievesToCheck) {
			...
			if (name != WeekData.getWeekFileName() + '_nomiss') // common achievements
			{
				switch(name)
				{
					...
					case 'toastie':
						// --- VOXEL ENGINE OPTIMIZATION: lowQualityStage ---
						unlock = (!ClientPrefs.data.cacheOnGPU && !ClientPrefs.data.shaders && (ClientPrefs.data.lowQuality || ClientPrefs.data.lowQualityStage) && !ClientPrefs.data.antialiasing);
